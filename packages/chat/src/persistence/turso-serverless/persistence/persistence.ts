...
export class TursoChatPersistence extends BaseChatPersistence implements ChatPersistence {
  ...
  async listThreads(input: ListThreadsInput): Promise<ThreadPage> {
    ...
    const rows = (await this.listThreadRows(input)) as ThreadRow[];
  ...
  private async listThreadRows(input: ListThreadsInput): Promise<unknown[]> {
    ...
    if (participant && input.channelId) {
      return all(
        ...
        [participant.type, participant.id, input.channelId],
      );
    }
    if (participant && input.postId) {
      return all(
        ...
        `SELECT t.*
         ...
         ORDER BY t.created_at_ms ASC`,
        [participant.type, participant.id, input.postId],
      );
    }
    if (participant) {
      return all(
  ...
  async startStreamedPost(input: import("../../../domain.ts").StartStreamedPostInput) {
    return startStreamedPost(this.db, input, () => null);
  }

  async applyPostDelta(input: import("../../../domain.ts").ApplyPostDeltaInput) {
    return applyPostDelta(this.db, input, () => null);
  }

  async completeStreamedPost(input: import("../../../domain.ts").CompleteStreamedPostInput) {
    return completeStreamedPost(this.db, input);
  }

  async abortStreamedPost(input: import("../../../domain.ts").AbortStreamedPostInput) {
