 ...
 */
const HASH_IMPORT_PREFIXES: Record<string, string> = {
...
export function fixDeclarationSpecifiers(filePath: string, distRoot: string): void {
  ...
  const rewritten = original.replace(
    ...
    (spec) => {
      for (const [prefix, underSrc] of Object.entries(HASH_IMPORT_PREFIXES)) {
        if (spec.startsWith(prefix)) {
          let rel = path.relative(
            path.dirname(filePath),
            path.join(distRoot, underSrc, spec.slice(prefix.length)),
          );
